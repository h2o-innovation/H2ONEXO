  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        {/* Header */}
        <div className="mb-20">
            <p className="text-sm font-mono text-primary mb-3">// CÓMO FUNCIONA</p>
          <h2
            className={`text-3xl lg:text-5xl font-semibold tracking-tight mb-6 transition-all duration-700 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
            }`}
          >
            <span className="text-balance">Un formulario.</span>
            <br />
            <span className="text-balance">Muchas posibilidades.</span>
          </h2>
        </div>
              ...
              <div className="p-6 font-mono text-sm min-h-[200px]">
                <pre className="text-muted-foreground">
                  {steps[activeStep].code.split('\n').map((line, i) => (
                    <span
                      key={`${activeStep}-${i}`} 
                      className="block leading-relaxed animate-in fade-in slide-in-from-left-2"
...
function highlightCode(line: string) {
  const tokenPattern = /(\/\/.*$|'[^']*'|"[^"]*"|\b(?:nexus|process|env)\b|\.\w+|[{}()[\]:])/g;
  ...
  for (const match of line.matchAll(tokenPattern)) {
    ...
    else if (token.startsWith(".")) className = "text-primary";
    else if (/^(nexus|process|env)$/.test(token)) className = "text-foreground";
    else className = "text-muted-foreground/70";
